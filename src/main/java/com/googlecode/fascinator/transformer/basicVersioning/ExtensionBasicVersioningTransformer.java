...
public class ExtensionBasicVersioningTransformer extends
	...
	public DigitalObject transform(DigitalObject in) throws TransformerException {
		try {
			...
			return transform(in, new JsonSimple(JsonSimpleConfig.getSystemFile()).getObject("transformerDefaults", getId()).toString());
		} catch (IOException e) {
	...
	public DigitalObject transform(DigitalObject in, String config)
        ...
        try {
            ...
            if (source.startsWith(".")) {
            	log.info("Versioning - Source not found at this time:'{}' from OID '{}'", source, in.getId());
            	return in;
            }
        ...
        try {
            ...
            try {
                ...
                sourcePayload.close(); 
                createVersionIndex(((FileSystemDigitalObject)in).getPath(), payloadName, source);
                return in;
            } catch (StorageException ex) {
